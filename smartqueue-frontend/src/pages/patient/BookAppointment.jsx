...
const BookAppointment = () => {
    ...
    // 1. Doctors state - backend se fetch hogi
    const [doctors, setDoctors] = useState([]);
    const [selectedDoctor, setSelectedDoctor] = useState(null);
    const [appointmentDate, setAppointmentDate] = useState('');
    const [reason, setReason] = useState('');

    // 2. Component load hote hi Port 8081 aur correct Admin endpoint se doctors laana
    useEffect(() => {
    ...
    // 3. Form submit handler - Backend par appointment save karna
        ...
        <div className="patient-dashboard-container">
            ...
            <aside className="patient-sidebar">
                <div className="patient-sidebar-top">
                    ...
                    <nav className="patient-nav">
                        <button onClick={() => navigate('/patient/dashboard')} className="patient-nav-btn">
                            📊 Dashboard
                        </button>
                        <button className="patient-nav-btn active">
                            📅 Book Appointment
                        </button>
                        <button
                            onClick={() => navigate('/patient/mytokens')}
                            className="patient-nav-btn"
                        >
                            🎫 My Tokens
                        </button>
                        <button onClick={() => navigate('/patient/appointmenthistory')} className="patient-nav-btn">
                            📜 History
                        </button>
                        <button onClick={() => navigate('/patient/profile')} className="patient-nav-btn">
                            👤 Profile
                        </button>
                    </nav>
                </div>
            ...
            <main className="patient-main">
                <header className="patient-header">
                    <h1 className="patient-header-title">Book Appointment</h1>
                ...
                <div className="patient-body" style={{ maxWidth: '800px' }}>
                    <div className="mb-6" style={{ marginBottom: '24px' }}>
                        <h2 className="patient-welcome-title">Select a Doctor & Schedule</h2>
                        <p className="patient-welcome-sub">Choose your preferred doctor and book your queue token instantly.</p>
                    </div>
                                    ...
                                    doctors.map((doc) => (
                                        <div
                                            ...
                                            style={{
                                                padding: '16px',
                                                borderRadius: '12px',
                                                border: selectedDoctor?.id === doc.id ? '2px solid #059669' : '1px solid #e5e7eb',
                                                backgroundColor: selectedDoctor?.id === doc.id ? '#ecfdf5' : '#ffffff',
                                                cursor: 'pointer',
                                                transition: 'all 0.2s'
                                            }}
                                        >
                                            <h3 style={{ fontSize: '16px', fontWeight: '600', color: '#1f2937' }}>{doc.name}</h3>
                                            <p style={{ fontSize: '13px', color: '#059669', fontWeight: '500', marginTop: '2px' }}>{doc.department || doc.specialization}</p>
                                            <p style={{ fontSize: '12px', color: '#6b7280', marginTop: '8px' }}>Fee: ₹{doc.consultationFee}</p>
                                            <p style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>Timing: {doc.availableTime}</p>
                                        </div>
                                    ))
                                ) : (
                                    <p style={{ color: '#6b7280', fontSize: '14px' }}>Loading doctors or no doctors available...</p>
                                )}
                            </div>
                        </div>
